...
interface CreateTabProps {
  language: Language;
  onGenerate: (content: string, type: QRType) => void;
}

type LogoShape = 'square' | 'rounded' | 'circle';

const CreateTab: React.FC<CreateTabProps> = ({ language, onGenerate }) => {
      ...
      qrImg.onload = () => {
        ...
        if (logo && selectedType !== 'barcode') {
          ...
          logoImg.onload = () => {
            ...
            ctx.beginPath();
            if (logoShape === 'circle') ctx.arc(x + size/2, y + size/2, size/2 + 20, 0, Math.PI * 2);
            ...
            ctx.beginPath();
            if (logoShape === 'circle') ctx.arc(x + size/2, y + size/2, size/2, 0, Math.PI * 2);
            else if (logoShape === 'rounded') {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: 'Smart Code', text: 'Shared from QR Generator BD' });
      } else {
    ...
    <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-slate-800">
        ...
        <div className="space-y-4">
          ...
          {selectedType !== 'barcode' && (
            <div className="pt-4 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                   {(['square', 'rounded', 'circle'] as LogoShape[]).map(shape => (
                     <button key={shape} onClick={() => setLogoShape(shape)} className={`w-7 h-7 rounded-lg border flex items-center justify-center transition-all ${logoShape === shape ? 'bg-blue-100 border-blue-500 text-blue-600' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                <button onClick={() => setLogo(null)} className={`flex-shrink-0 w-12 h-12 rounded-xl border-2 flex items-center justify-center transition-all ${!logo ? 'border-blue-500 bg-blue-50' : 'border-transparent bg-slate-50 dark:bg-slate-800'}`}><i className="fa-solid fa-ban text-gray-400"></i></button>
                ...
                <button onClick={() => fileInputRef.current?.click()} className="flex-shrink-0 w-12 h-12 rounded-xl border-2 border-dashed border-blue-300 flex items-center justify-center bg-blue-50 text-blue-600"><i className="fa-solid fa-plus"></i></button>
          ...
          <button onClick={handleGenerate} className="w-full bg-blue-600 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 active:scale-95 transition-all text-sm mt-4 flex items-center justify-center gap-3">
      ...
      {generated && (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-2xl flex flex-col items-center animate-in zoom-in duration-500 border border-slate-100 dark:border-slate-800">
           <div className={`relative p-6 bg-white rounded-[2.5rem] shadow-2xl mb-6 overflow-hidden border border-slate-50 ${selectedType === 'barcode' ? 'w-full flex items-center justify-center' : ''}`}>
              <img src={getQRUrl(350)} alt="Code" className={`${selectedType === 'barcode' ? 'max-w-full h-auto' : 'w-60 h-60'} block`} />
           ...
           <div className="flex gap-3 w-full">
              <button onClick={handleDownload} disabled={isProcessing} className="flex-1 bg-slate-900 dark:bg-blue-600 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all disabled:opacity-50"><i className={`fa-solid ${isProcessing ? 'fa-spinner fa-spin' : 'fa-download'}`}></i> {t.download}</button>
              <button onClick={handleShare} disabled={isProcessing} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700 active:scale-95 transition-all"><i className="fa-solid fa-share-nodes"></i> {t.share}</button>
           </div>
        </div>
      )}
    </div>
